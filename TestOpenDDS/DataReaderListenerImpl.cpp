		...
		if (ret == DDS::RETCODE_OK)
		{
			...
			for (int i = 0; i < sampleInfoLength; ++i)
			{
				if (sampleInfos[i].valid_data)
				{
					...
					tmpLog.Format(L"¶©ÔÄ£ºDataA name:%s pos:%d length:%d", CString(dataA[i].name), dataA[i].pos, dataA[i].length);
	...
	else if (topicName == "TestA::DataB")
	{
		...
		if (ret == DDS::RETCODE_OK)
		{
			...
			for (int i = 0; i < sampleInfoLength; ++i)
			{
				if (sampleInfos[i].valid_data)
				{
					tmpLog.Format(L"¶©ÔÄ£º DataB name:%s id:%d longitude:%lf", CString(dataB[i].name), dataB[i].id, dataB[i].longitude);
					_dlg->AppendMSG(tmpLog);
				}
			}
		}
